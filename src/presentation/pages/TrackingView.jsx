...
import { PackageSearch, AlertCircle, Phone, RefreshCcw, Download, Lock, KeyRound, Star, LayoutDashboard } from 'lucide-react';
...
export default function TrackingView() {
            ...
            <div className="max-w-md mx-auto mt-20 bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden animate-in zoom-in-95 duration-300">
                ...
                <div className="p-8 text-center relative">
                    <div className="animate-in fade-in slide-in-from-right-4">
                        ...
                        <h2 className="text-2xl font-extrabold text-slate-900 mb-2 tracking-tight">CEO 파트너 대시보드</h2>
                        <p className="text-slate-500 mb-8 text-sm">아임오케이 비즈니스 파트너 전용 공간입니다.<br />발급받으신 파트너 코드를 입력해주세요.</p>
                        <form onSubmit={handleAuthSubmit} className="space-y-4">
                            <div className="relative">
                                ...
                                <input
                                    type="text"
                                    className="w-full pl-11 pr-4 py-3.5 rounded-xl border border-slate-300 focus:ring-2 focus:ring-orange-500 outline-none font-bold tracking-widest uppercase text-center text-slate-900 bg-slate-50 focus:bg-white transition-colors"
                                    placeholder="파트너 코드 입력 (예: CB-9021)"
                                    value={authCode}
                            ...
                            <button type="submit" className="cursor-pointer w-full bg-slate-900 text-white px-6 py-4 rounded-xl font-bold hover:bg-slate-800 transition-all shadow-md hover:shadow-lg">
                                대시보드 접속하기
                            </button>
                        </form>
                    </div>
    ...
    return (
        <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6">
            <div className="mb-10 flex flex-col md:flex-row items-center justify-between bg-slate-900 text-white p-8 rounded-2xl shadow-xl">
                <div className="flex items-center mb-6 md:mb-0">
                    <div className="w-16 h-16 bg-slate-800 rounded-2xl flex items-center justify-center mr-6 border border-slate-700 shadow-inner">
                        <LayoutDashboard className="h-8 w-8 text-orange-400" />
                    </div>
                    <div>
                        <h2 className="text-3xl font-extrabold tracking-tight mb-1">{academyName} 원장님, 환영합니다.</h2>
                        <p className="text-slate-400 font-medium">아임오케이 비즈니스 파트너 전용 대시보드</p>
                    </div>
                </div>
                <div className="text-sm border border-slate-700 bg-slate-800 px-4 py-3 rounded-xl flex items-center shadow-inner">
                    <span className="text-slate-400 mr-2">파트너 코드:</span>
                    <span className="font-bold text-orange-400 tracking-wider">
            ...
            <div className="space-y-6">
                <h3 className="text-xl font-bold flex items-center mb-6 border-b border-slate-200 pb-4">
                    주문 및 작업 현황 <span className="ml-2 bg-slate-100 text-slate-600 text-sm px-3 py-1 rounded-full font-bold">{activeOrders.length}건</span>
                </h3>

                {activeOrders.length === 0 ? (
                    <div className="bg-slate-50 rounded-2xl p-12 text-center border border-slate-200">
                        <AlertCircle className="w-12 h-12 text-slate-400 mx-auto mb-4" />
                        <p className="text-slate-600 font-medium text-lg">진행 중인 주문 내역이 없습니다.</p>
                        <p className="text-slate-400 mt-2 text-sm">새로운 디자인 작업을 요청해주시면, 이곳에서 쉽고 빠르게 관리할 수 있습니다.</p>
                        <button
                            onClick={() => navigate('/order')}
                            className="mt-6 bg-white border border-slate-300 text-slate-700 px-6 py-3 rounded-lg font-bold hover:bg-slate-50 transition-colors shadow-sm"
                        >
                            신규 디자인 의뢰하기
                        </button>
                    </div>
                                    ...
                                    {order.status === 'WAIT_CONFIRM' && (
                                        <div className="bg-orange-50 border border-orange-200 rounded-xl p-6 text-center shadow-inner">
                                            ...
                                            <div className="flex gap-3 justify-center">
                                                <button onClick={() => handleStatusUpdate(order.id, 'APPROVED')} className="cursor-pointer bg-slate-900 text-white px-6 py-3 rounded-lg font-bold hover:bg-slate-800 transition-colors shadow-md min-w-[140px]">
                                                    시안 최종 확정
                                                </button>
                                                <a href="http://pf.kakao.com/_xxxxxx/chat" target="_blank" rel="noreferrer" className="cursor-pointer bg-yellow-400 text-yellow-950 px-6 py-3 rounded-lg font-bold hover:bg-yellow-500 transition-colors shadow-md flex items-center min-w-[140px] justify-center">
                                                    카톡으로 수정 요청
                                                </a>
                                            </div>
                                        </div>
                                    ...
                                    {['DESIGN', 'APPROVED', 'SHIPPING', 'DONE'].includes(order.status) && (
                                        <div className="flex gap-4 items-center p-4 bg-slate-50 border border-slate-200 rounded-xl">
                                            ...
                                            <div className="text-sm">
                                                ...
                                                <p className="text-slate-500">순조롭게 진행 중입니다. 확인이 필요한 사항은 카카오톡으로 안내해 드립니다.</p>
                                            </div>
                                        </div>
                                    )}
